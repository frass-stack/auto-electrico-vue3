<template>
  <div id="app" :class="{ 'dark-mode': themeStore.isDarkMode }">
    ...
    <template v-else>
      ...
      <CinturonAlert :show="showCinturonAlert" @close="showCinturonAlert = false" />
    </template>
  </div>
</template>
...
import CinturonAlert from './components/CinturonAlert.vue'
...
export default {
  name: 'App',
  components: { CinturonAlert },
  setup() {
    const isMenuOpen = ref(false)
    const themeStore = useThemeStore()
    const authStore = useAuthStore()
    const router = useRouter()
    const showCinturonAlert = ref(false)
    ...
    const handleLogout = async () => {
      ...
      console.log('🔍 Logout completed, reloading page to force navigation')
    ...
    onMounted(() => {
      mqttService.subscribe('/CinturonConductor', (payload) => {
        let valor = typeof payload === 'string' ? payload : payload?.estado || payload?.value || payload
        if (typeof valor === 'string') valor = valor.trim()
        showCinturonAlert.value = (valor === 'Pendiente')
        console.log('Mensaje recibido en /CinturonConductor:', payload)
      })
      mqttService.subscribe('/MedidorDeBateria', (payload) => {
        console.log('Mensaje recibido en /MedidorDeBateria:', payload)
      })
      mqttService.subscribe('/Velocidad', (payload) => {
        console.log('Mensaje recibido en /Velocidad:', payload)
      })
    })
